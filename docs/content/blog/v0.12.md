...
Right-click on any page to copy its content as Markdown or view the raw Markdown source. As more users feed documentation into generative AI tools, having quick access to clean Markdown is increasingly useful. Custom links with template variables (`{url}`, `{title}`, `{markdownUrl}`) let you wire up "Ask AI" shortcuts or other integrations directly in the menu.
...
- Content width respects `page.width` config
